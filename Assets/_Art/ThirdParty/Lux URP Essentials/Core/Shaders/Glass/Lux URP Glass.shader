...
Shader "Lux URP/Glass"
{
    Properties
    {
        ...
        [Enum(Off, 0, Multiply, 1, Additive, 2, AlphaBlend, 3)]
        _BaseBlendMode ("BaseMap Blending Mode", Float) = 0.0
    ...
    SubShader
    {
        ...
        // Declare the _BaseBlendMode property
        ...
        //  Fragment shader and functions
            ...
            )
            {
                ...
                half3 baseMapColor = albedoAlpha.rgb * _BaseColor.rgb;
                
                // Apply the blending mode
                if (_BaseBlendMode == 1.0) {
                    outSurfaceData.albedo = baseMapColor * _BaseColor.rgb * _Multiplier;
                } else if (_BaseBlendMode == 2.0) {
                    outSurfaceData.albedo = baseMapColor + _BaseColor.rgb;
                } else if (_BaseBlendMode == 3.0) {
                    outSurfaceData.albedo = lerp(_BaseColor.rgb, baseMapColor, albedoAlpha.a);
                } else {
                    outSurfaceData.albedo = baseMapColor;
                }
            #else
