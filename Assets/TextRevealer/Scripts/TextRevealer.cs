...
public class TextRevealer : MonoBehaviour {
    ...
    [FormerlySerializedAs("RevealDelaySeconds")] public float RevealOnEnableDelaySeconds = 0f;
    ...
    private void OnEnable()
    {
        ...
        {
            Invoke("Reveal", RevealOnEnableDelaySeconds);
        }
    }
    ...
    public void RevealVoid(int delaySeconds = 0)
    {
        Invoke("Reveal", delaySeconds);
    }

    public void UnrevealVoid(int delaySeconds = 0)
    {
        Invoke("Unreveal", delaySeconds);
    }
    ...
    Animation CreateRevealAnimation(GameObject slicedParent, List<Transform> slices)
    {
        ...
        while (i != iEnd)
        {
            ...
            if (RevealSmoothOverlap)
            {
                startTimeForSlice = ((float)(RevealReverse ? slices.Count - i - 1 : i) / (float)(slices.Count == 1 ? 1 : slices.Count - 1)) * (RevealTime - revealTimePerSlice);
            }
            else
            {
    ...
    Animation CreateUnrevealAnimation(GameObject slicedParent, List<Transform> slices)
    {
        ...
        while (i != iEnd)
        {
            ...
            if (UnrevealSmoothOverlap)
            {
                startTimeForSlice = ((float)(UnrevealReverse ? slices.Count - i - 1 : i) / (float)(slices.Count == 1 ? 1 : slices.Count - 1)) * (UnrevealTime - unrevealTimePerSlice);
            }
            else
            {
