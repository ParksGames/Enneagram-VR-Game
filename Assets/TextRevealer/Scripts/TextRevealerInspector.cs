...
public class TextRevealerEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
            ...
            || myScript.textComponent is TextMeshPro))
        {
            ...
            if (myScript.RevealOnEnabled)
            {
                myScript.RevealOnEnableDelaySeconds = EditorGUILayout.FloatField(new GUIContent("Reveal on enable delay (seconds)"),
                    myScript.RevealOnEnableDelaySeconds);
                myScript.RevealOnEnableDelaySeconds = Math.Max(myScript.RevealOnEnableDelaySeconds, 0);
            }
